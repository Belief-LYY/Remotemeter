...
<body background="#fafafa;">
	...
	<div
		...
		<ol class="breadcrumb" style="font-size: 15px;">
			...
			<li><a style="color: #4F4F4F;" href="#">新 增></a></li>
		</ol>


		<div class="col-xs-3"
			style="float: right; margin-right: 120px; letter-spacing: 3px; width: 220px; font-size: 20px; margin-top: 60px;">
			<a style="font-size: 19px;" class="btn btn-block btn-lg btn-warning"
				onClick="link();">提交 | Submit</a>
		</div>
		...
		<div
			...
			<div class="col-xs-3"
				...
				<nobr>
					*方案编号： <input type="text" id="sche" name="sche"
						style="display: inline; width: 260px;" value="" placeholder=""
						class="form-control" />
				</nobr>
				<br>
				<hr>
...
<script type="text/javascript">
	function link() {
		...
		if (sche.length == 0)
			...
			alert("请选择起始时间！");
		else if (start >= end && end.length != 0)
			alert("起始时间应小于终止时间！");
		else if (!confirm("确认添加？")) {
			window.event.preventDefault();
			//window.event.returnValue = false;
		} else {
			console.log("rate:" + rate + ";start:" + start + ";sche:" + sche
					+ ";end:" + end);
			xmlhttp.open("POST", "/Remotemeter/servlet/schemeServlet?sche="
					+ sche + "&rate=" + rate + "&start=" + start + "&end="
					+ end, true);
			xmlhttp.send();
