...
<body background="#fafafa;">
	...
	<div
		...
		<ol class="breadcrumb" style="font-size: 15px;">
			...
			<li><a style="color: #4F4F4F;" href="#">通 断 电 控 制 ></a></li>
		</ol>
		...
		<div class="col-xs-3"
			...
			<form action="/Remotemeter/servlet/powerServlet"
				method="post">
				...
				<div class="col-xs-3"
					style="float: left; margin-left: 400px; letter-spacing: 3px; width: 200px; font-size: 20px; margin-top: 0px;">
					<label class="radio"> <input type="radio"
						name="optionsRadios" id="optionsRadios1" value="option1" onClick="onclif();"
						data-toggle="radio" checked> 全部显示
					</label> <label class="radio"> <input type="radio"
						name="optionsRadios" id="optionsRadios2" value="option1" onClick="oncli();"
						data-toggle="radio"> 仅显示欠费电表
					</label>
				</div>
					...
					<table class="table table-hover"
						...
						<tbody>
							...
							<%
								...
								MyaccountDAO imyaccountdao=new MyaccountDAO();
								ArrayList<personal> ipersonals=new ArrayList<personal>();
								ArrayList<company> icompanys=new ArrayList<company>();
								...
								if (list != null && list.size() > 0) {
									for (int i = 0; i < list.size(); i++) {
										...
										{
											type="商用";
											icompanys=imyaccountdao.companyinfo(imeter.getId());
											if(icompanys.size()!=0)
											icompany=icompanys.get(0);
											phone=icompany.getPhone();
										}
										if(imeter.getTypen()==1)
										{
											type="民用";
											ipersonals=imyaccountdao.personalinfo(imeter.getId());
											if(ipersonals.size()!=0)
											ipersonal=ipersonals.get(0);
											phone=ipersonal.getPhone();
										}
										String state="";
										if(imeter.getState().equals("通电"))
											state="checked";
										System.out.println("5555"+state);
							%>
							<tr>
								<td><a style="color: #73BAA9;"
									href="/Remotemeter/servlet/powerServlet?meter_id=<%=imeter.getMeter_id()%>&action=meter"><%=imeter.getMeter_id()%></a></td>
								<td><a style="color: #73BAA9;"
									href="/Remotemeter/servlet/powerServlet?id=<%=imeter.getId()%>&action=user"><%=imeter.getId()%></a></td>
								<td><%=type%></td>
								<td><%=imeter.getRemark()%></td>
								<td><%=imeter.getState()%></td>
								<td><%=phone%></td>
								<td><%=imeter.getBalance()%></td>
								<td><input onChange="power('<%=imeter.getMeter_id()%>');"
									type="checkbox" <%=state%> data-toggle="switch" id="custom-switch-02" /></td>
							</tr>
